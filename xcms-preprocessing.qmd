---
title: "xcms-based preprocessing of a large LC-MS/MS data set"
format: html
...
We next perform the peak refinement to reduce peak detection artifacts.
...
write.table(fall, "xcms_all_features.txt", sep = "\t",
...
tmp <- peakRAM(
    export(backend = MsBackendMgf(),
           ms2_maxtic, file = "ms2spectra_maxTic.mgf")
)
pr_ms2 <- rbind(pr_ms2, tmp)
```
...
Framework 13' notebook with a 13th Gen Intel(R) Core(TM) i7-1370P CPU (20 cores)
