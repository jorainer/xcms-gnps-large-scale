---
title: "xcms-based preprocessing of a large LC-MS/MS data set, MSnbase infrastructure"
output: pdf_document
...
consists of approximately 1,000 samples (mzXML) files which can be downloaded
from the *MassIVE* repository. The accession ID of the data set is
...
from this file on newer versions of R/Bioconductor, respectively versions of the
*xcms* package will most likely result in errors. The present analysis was run
using the
...
Bioconductor docker image (install with `docker pull
...
The size of the object in memory is shown below. Note that only spectra metadata
is loaded into memory, while the mass peak data is retrieved on-the-fly from the
original data files.
